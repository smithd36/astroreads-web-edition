...
const links = ["Home", "Recommendations", "Library"];
    ...
    return (
        <div>
            <nav className="bg-gray-800 flex justify-between items-center h-15 p-4 z-10">
                ...
                <p className="text-gray-200 ml-4 font-thin text-2xl mr-auto"><a href="/home">AstroReads</a></p>

                {/* NavBar Links */}
                <ul className="flex gap-6 list-none text-gray-100">
                    {links.map((link) => 
                    <li key={link}>
                        <a href={link.toLowerCase()} className="font-thin hover:bg-slate-700 p-2 hover:rounded-md active:bg-sky-950 focus:ring focus:ring-sky-800 focus:rounded-md focus:outline-none">
                        {link}
                        </a>
                    </li>
                    )}
